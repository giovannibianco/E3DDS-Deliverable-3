...
IT service management (ITSM) %define and reference
...
% log, metrics (rdd, ganglia), collection, graphana
https://www.rocksclusters.org/
Monitoring tools: ?%nav https://nav.uninett.no/ %% Dan has stuff here.

\subsubsection{logging} \label{ssec:logging}
Logging: syslog/rsyslog, other tools? %% Dan has stuff here.
...
%kernel updates important for security reasons
%ringbuffer nodes code be target if used for user compute
%beamformer nodes probably no user compute
...
\bitm
% long term support, existing experience of staff
...
This can be important for getting help from others when problems arise.
The choice of OS should weigh these considerations to find the most useful solution.
%% John... personal choice.
\item BIOS settings: \\% security, vulnerabilities?
%% Unknown?
%% Vendor? ... depends on ... 
...
\todo[inline,color=red]{JW: Comment on ``open-source'' vs. proprietary dhcp servers? i.e. proprietary may provide gui for admin but tables are stored in binary format. possible loss of data over the years. and ``open-source'' dhcp server that stores tables in plain format much more resilient to data loss and easier to migrate. back up dhcp server? dhcp server to run in VM cluster? dhcp server for each subnet?}

The sub-networks can also be configured manually with static IP addresses assigned to each ethernet port.
\todo[inline,color=red]{JW:This strategy is ... NOT GOOD!}
...
%% Usually by the vendor
%% GUI to change settings.
%% Ended up with different O/S versions.
%% Layers and balancing etc.
